...
async def create_player(
    ...
    player_photo: Optional[UploadFile] = File(
        None, description="Player photo (optional)"
    ),
...
async def get_player_info(
    player_id: int, player_service: PlayerService = Depends(get_player_service)
):
...
async def update_player_info(
    ...
    player = await player_service.update_player(
        player_id, player_data.dict(), player_photo
    )
    return PlayerResponse.from_orm(player)
