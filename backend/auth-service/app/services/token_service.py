...
from app.utils.crypto import decode_jwt, create_jwt_token
...
class TokenService:
    ...
    async def generate_access_token(self, user_id: int) -> str:
        expires_at = (datetime.now(timezone.utc) + timedelta(minutes=int(settings.ACCESS_TOKEN_EXPIRE_TIME_MINUTES))).replace(tzinfo=None)
    ...
    async def generate_refresh_token(self, user_id: int) -> str:
        expires_at = (datetime.now(timezone.utc) + timedelta(minutes=int(settings.REFRESH_TOKEN_EXPIRE_TIME_MINUTES))).replace(tzinfo=None)
